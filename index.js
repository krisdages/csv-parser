...
class CsvParser extends Transform {
  constructor (opts = {}) {
    ...
    if (options.headers || options.headers === false) {
      state.first = false
      if (options.getMapValue && Array.isArray(options.headers)) {
        state.cellMapValues = [];
        const colEnd = options.headers.length
        const passthru = val => val
        for (let i = 0; i < colEnd; i++) {
          state.cellMapValues.push(options.getMapValue({ header: options.headers[i], index: i }) || passthru)
        }
      }
    }
  ...
  parseLine (buffer, start, end) {
    ...
    const mapValue = state.cellMapValues
      ? (value) => {
        if (state.first) {
          return value
        }
    ...
    if (state.first && !skip) {
      ...
      if (getMapValue && !state.cellMapValues) {
        state.cellMapValues = [];
